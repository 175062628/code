 ...
 *  性质1：根为B
 *	性质2：R的孩子节点只能为B
 *	性质3：根节点到任意叶子节点所经过的B节点数量相同
 *  性质4：叶子节点(nil)为B 凡是插入的节点，其孩子节点默认为nil
 */
class RB_Tree{
	...
	struct TreeNode{
		...
		TreeNode(int v,int c):val(v),color(c){}
		TreeNode(int v,TreeNode* l=nullptr,TreeNode* r=nullptr,TreeNode* p=nullptr):val(v),color(RED),left(l),right(r),parent(p){}
...
private:
	 ...
	 * 											      
	 */
	void insertCheck1(TreeNode*& cur){
		// 没有父节点的情况
		if(!cur->parent){
			cur->color=BLACK;
			root=cur;
	 ...
	 * 
	 */
	void insertCheck2(TreeNode*& cur){
		// case 1
		if(!cur->parent){
			cur->color=BLACK;
			root=cur;
	...
	void insert(TreeNode*& cur,int val){
		if(!cur){
			TreeNode* t=new TreeNode(val,nil,nil);
			insertCheck2(t);
			return;
		}
		if(val<= cur->val){
			if(cur->left!=nil){
				...
				TreeNode* t=new TreeNode(val,nil,nil,cur);
				cur->left=t;
				insertCheck2(t);
			}
		}
	 ...
	 */
	// cur是替换节点
	void eraseCase(TreeNode*& cur){
		// case 1
		if(!cur->parent){
			root=cur;
			return;
		}
		TreeNode *p=cur->parent,*s=cur->sibling(),*sl=s->left,*sr=s->right;
		// case 2
	...
	void eraseHelp(TreeNode* cur){ // 引用传递就会报错，原因在于引用传递会在原地址上修改，即parent->child==cur
		TreeNode* child=cur->left!=nil?cur->left:cur->right;
	...
	void erase(TreeNode*& cur,int val){
		if(cur==nil) return;
		if(val< cur->val){
			erase(cur->left,val);
		}
			...
			TreeNode* next=getMin(cur->right);
			cur->val=next->val;
			eraseHelp(next);
		}
	}
	...
	// cur是旋转完的根节点
	void RotateR(TreeNode*& cur){
		TreeNode *parent=cur->parent,*grandparent=cur->grandparent(),*right=cur->right;
		// 修改右节点
	...
	void RotateL(TreeNode*& cur){
		TreeNode *parent=cur->parent,*grandparent=cur->grandparent(),*left=cur->left;
		// 修改left
	...
	TreeNode *root=nullptr,*nil=new TreeNode(0,BLACK);
};

int main(){
	vector<int> nums={2,5,7,23,4,1,6,9,8}; 
	RB_Tree tree(nums);
	tree.dfs();
	tree.erase(4);
}
